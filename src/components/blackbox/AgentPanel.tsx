      ...
      <div className="sticky top-0 z-10 flex items-center justify-between border-b border-border bg-card px-4 py-3.5">
        <div className="flex items-center gap-2.5">
          <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-primary">
            <Cpu className="h-4 w-4 text-primary-foreground" strokeWidth={2} />
          </div>
          <div className="leading-tight">
            <p className="text-[12px] font-semibold uppercase tracking-wide text-foreground">
              Blackbox Agent
            </p>
            <p className="text-[12px] text-muted-foreground">Diagnosis ready</p>
          </div>
        </div>
      ...
      <div className="flex flex-col gap-4 p-4">
        ...
        <section className="flex flex-col gap-1.5">
          ...
          <p className="text-[13px] leading-relaxed text-muted-foreground">
            ...
            module output only{" "}
            <span className="font-medium text-anomaly">9.7°</span> because steering was clamped
            before curvature compensation.
          </p>
        </section>

        {/* Fix */}
        <section className="rounded-xl border border-replay/25 bg-replay-soft/60 p-3">
          <p className="text-[11px] font-semibold uppercase tracking-wide text-replay">Fix</p>
        ...
        <section className="rounded-xl border border-border bg-surface-muted p-3.5">
          ...
          <div className="mt-2 rounded-lg border border-border bg-card px-2.5 py-2 font-mono text-[11.5px] leading-relaxed">
            <p className="text-foreground">{ROOT_FILE}</p>
            <p className="text-muted-foreground">{ROOT_FN}</p>
          </div>
          <div className="mt-2.5 flex items-center gap-1.5">
            <span className="rounded-md border border-border bg-card px-1.5 py-0.5 text-[11px] font-medium text-muted-foreground">
              Control
            </span>
            <span className="rounded-md border border-success/30 bg-success-soft px-1.5 py-0.5 text-[11px] font-medium text-success">
              Patch ready
            </span>
          </div>
        ...
        <section className="grid grid-cols-2 gap-2">
          {METRICS.map((m) => (
            <div
              key={m.label}
              className="rounded-[10px] border border-border bg-surface-muted px-3 py-2.5"
            >
              <p className="text-[12px] text-muted-foreground">{m.label}</p>
              <p className="mt-0.5 text-[14px] font-semibold text-foreground">{m.value}</p>
            </div>
          ))}
        </section>
          ...
          {!allDone &&
            AGENT_STEPS.slice(visibleSteps, visibleSteps + 1).map((step) => (
              <div key={step} className="flex items-center gap-2 py-1">
                ...
                <span className="text-[12.5px] text-muted-foreground">{step}</span>
              </div>
            ))}
        </section>
        ...
        <div className="flex flex-col gap-2.5 pt-1">
          <Button className="h-10 w-full gap-1.5" onClick={onApply}>
            Apply in Cursor
